...
function initTool () {
  sankeyConfigs.push({ sankeyDataID: '', sankeyInstanceID: 'energyflows', targetDIV: 'SVGContainer_energyflows', width: 1600, height: 1200})
  ...
  if (dataSource == 'url') {
    readExcelFile(XLSXurl, (rawSankeyData) => {
      ...
      sankeyConfigs.forEach(element => {
        ...
        var legend = rawSankeyData.carriers[config.sankeyDataID]
        ...
        currentUnit = settings[0].unit || 'Unit undefined'
        // Also set the global window variable
...
function readExcelFile (url, callback) {
  ...
  xhr.onload = () => {
    ...
    console.log('Stored original workbook from URL with sheets:', workbook.SheetNames)
    
    // Store the original filename from URL for export purposes
...
function generateSankeyLibrary (workbook) {
  ...
  workbook.SheetNames.forEach(sheetName => {
    ...
    if (worksheet) {
      ...
      // Also handle the old snky_ prefix format for backwards compatibility
        ...
        if (after && ['links', 'nodes', 'remarks', 'carriers', 'settings'].includes(after)) {
          // Ensure the top-level object exists
  ...
  // console.log('Generated sankeyDataLibrary:', sankeyDataLibrary)
...
// }, 1000)
 ...
 decryptButton.addEventListener('click', async () => {
   ...
   try {
       ...
       if (!zipFile.dir) {
         if (excelExtensions.test(fileName)) {
           ...
           try {
            ...
            // Store the complete original workbook if this is the sankeyfy_input file
            if (baseName === 'sankeyfy_input') {
              window.originalWorkbook = workbook;
              // Store the original filename for export purposes
              window.originalFilename = fileName.split('/').pop(); // Extract just the filename
              console.log('Stored original filename from ZIP:', window.originalFilename);
            }
           } catch (err) {
     ...
     // Removed cijferbasis initialization for simplified version
     
     // Removed waterfall dataset loading and initialization for simplified version

    //  alert('Excel data extracted — check the console!');

     // Store the original workbook before it gets processed
     const preservedOriginalWorkbook = window.originalWorkbook;
     console.log('Preserved original workbook with sheets:', preservedOriginalWorkbook ? preservedOriginalWorkbook.SheetNames : 'none');
     
     // GENERATE SANKEY
     var rawSankeyData = generateSankeyLibrary(jsonToWorkbook(excelData.sankeyfy_input))
     
     // Restore the original workbook after processing (jsonToWorkbook creates a new one)
     if (preservedOriginalWorkbook) {
       window.originalWorkbook = preservedOriginalWorkbook;
       console.log('Restored original workbook with sheets:', window.originalWorkbook.SheetNames);
     } else {
       console.log('No original workbook to restore from ZIP loading process');
     }
